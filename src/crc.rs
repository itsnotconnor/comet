...
mod tests {
    ...
    /* 
    Reference check for pre-computed CRC16 (https://crccalc.com/?crc=cafebeefbeefcafe&method=CRC-16/AUG-CCITT&datatype=hex&outtype=0)
    */

    #[test]
    fn test_crc_16_table() {
        // CAFE BEEF BEEF CAFE
        let input_bytes : [u8;8] = [0xCA, 0xFE, 0xBE, 0xEF, 0xBE, 0xEF,0xCA, 0xFE];
        let expected_crc : u16 = 0x6B3B;

        assert_eq!(crc16_table(&input_bytes), expected_crc);
    }

    #[test]
    fn test_crc_16_manual() {
        // CAFE BEEF BEEF CAFE
        let input_bytes : [u8;8] = [0xCA, 0xFE, 0xBE, 0xEF, 0xBE, 0xEF,0xCA, 0xFE];
        let expected_crc : u16 = 0x6B3B;
